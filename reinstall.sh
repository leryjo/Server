...
is_mbr_using_grub() {
    ...
    head -c 440 /dev/$sda | grep --text -iq 'GRUB'
}
...
find_main_disk() {
    ...
    if is_in_windows; then
        ...
        sda=$(lsblk -rn --inverse $mapper | grep -w disk | awk '{print $1}' | sort -u)

        # 检测主硬盘是否横跨多个磁盘
        os_across_disks_count=$(wc -l <<<"$sda")
        if [ $os_across_disks_count -eq 1 ]; then
            info "Main disk: $sda"
        else
            error_and_exit "OS across $os_across_disks_count disk: $sda"
        fi
        ...
        # 获取 sda 的 id
        install_pkg fdisk
        main_disk=$(fdisk -l /dev/$sda | grep 'Disk identifier' | awk '{print $NF}' | sed 's/0x//')
    fi
...
EOF
    ...
    if [ $ram_size -gt 256 ] || is_in_windows; then
        ...
        find_main_disk
        extra_drivers=
        for driver in $(get_disk_drivers $sda); do
...
exit_if_cant_use_cloud_kernel() {
    ...
    # shellcheck disable=SC2154
    if ! can_use_cloud_kernel "$sda" $ipv4_ethx $ipv6_ethx; then
